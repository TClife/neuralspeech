...
class Tacotron2Loss(nn.Module):
    ...
    def forward(self, model_outputs, targets):
        ...
        output_lengths.requires_grad = False #freezing as to no change in parameters
...
class Encoder(nn.Module):
    ...
    def forward(self, x, input_lengths): #input_lengths = maximum length
        for conv in self.convolutions:
            x = F.dropout(F.relu(conv(x)), 0.5, self.training)

        x = x.transpose(1, 2) #[4, 141, 512]
        ...
        self.lstm.flatten_parameters() #for data parllel 
...
class Decoder(nn.Module):
    ...
    def get_go_frame(self, memory):
        ...
        '''
        B = memory.size(0) #batch size
        decoder_input = Variable(memory.data.new(
            B, hps.num_mels * hps.n_frames_per_step).zero_()) #n_frames_per_step: Number of frames processed per step
    ...
    def parse_decoder_inputs(self, decoder_inputs):
        ...
        decoder_inputs = decoder_inputs.transpose(1, 2).contiguous() #[4, 792, 80]
        decoder_inputs = decoder_inputs.view(
            decoder_inputs.size(0),
            int(decoder_inputs.size(1)/hps.n_frames_per_step), -1) #[4, 264, 240]
        # (B, T_out, num_mels) -> (T_out, B, num_mels)
        decoder_inputs = decoder_inputs.transpose(0, 1) #[264, 4, 240]
    ...
    def parse_decoder_outputs(self, mel_outputs, gate_outputs, alignments):
        ...
        mel_outputs = torch.stack(mel_outputs).transpose(0, 1).contiguous() #[1, 48, 240]
        # decouple frames per step
        mel_outputs = mel_outputs.view(
            mel_outputs.size(0), -1, hps.num_mels) #[1, 144, 80]
    ...
    def forward(self, memory, decoder_inputs, memory_lengths):
        ...
        '''
        decoder_input = self.get_go_frame(memory).unsqueeze(0) #[1, 4, 240] which is [1, bs, num_mels * n_frames per step] all zero frame
        decoder_inputs = self.parse_decoder_inputs(decoder_inputs) #[264, 4, 240] (t_out, b, num_mels)
        decoder_inputs = torch.cat((decoder_input, decoder_inputs), dim=0) #[265, 4, 240]
        decoder_inputs = self.prenet(decoder_inputs) #[265, 4, 256]
        ...
        while len(mel_outputs) < decoder_inputs.size(0) - 1:
            ...
            mel_output, gate_output, attention_weights = self.decode(
                decoder_input)
            mel_outputs += [mel_output.squeeze(1)] #[4, 240] predicting mel for three time steps hence (80*3)
            gate_outputs += [gate_output.squeeze()] #predicting stop tokens
            alignments += [attention_weights] #[4, 117] atention weights in each of the text tokens
        mel_outputs, gate_outputs, alignments = self.parse_decoder_outputs(
            mel_outputs, gate_outputs, alignments)
        return mel_outputs, gate_outputs, alignments #[4, 80, 792], [4, 264] [4, 264, 117]
...
class Tacotron2(nn.Module):
    ...
    def parse_batch(self, batch):
        ...
        text_padded = mode(text_padded).long() #[bs, 117] text padded to longest text
        input_lengths = mode(input_lengths).long() #length of each text 
        max_len = torch.max(input_lengths.data).item()
        mel_padded = mode(mel_padded).float() #[bs, 80, 792] mel-spectrogram padded to longest mel
        gate_padded = mode(gate_padded).float() #zero for non-padded values, 1 for padded values
        output_lengths = mode(output_lengths).long() #shape of non-padded value time-sequence
        return (
    ...
    def forward(self, inputs):
        text_inputs, text_lengths, mels, max_len, output_lengths = inputs #[4, 117], [117, 101, 54, 43], [4, 80, 792], 117, [792, 544, 313, 223] 
        text_lengths, output_lengths = text_lengths.data, output_lengths.data

        embedded_inputs = self.embedding(text_inputs).transpose(1, 2) #[bs, text_length, dim].transpose(1,2) -> [bs, 512, 141] in my case this would be different

        encoder_outputs = self.encoder(embedded_inputs, text_lengths) #[4, 117, 512] 

        mel_outputs, gate_outputs, alignments = self.decoder( # gate_outputs = [bs, 251], alignments = [bs, 251, 141]
            encoder_outputs, mels, memory_lengths=text_lengths)

        mel_outputs_postnet = self.postnet(mel_outputs)
        mel_outputs_postnet = mel_outputs + mel_outputs_postnet #refining mel_outputs to improve quality [4, 80, 792]
        return self.parse_output(
            [mel_outputs, mel_outputs_postnet, gate_outputs, alignments],
            output_lengths)

    def inference(self, inputs):
        self.embedding, inputs = self.embedding.cuda(), inputs.cuda()
        embedded_inputs = self.embedding(inputs).transpose(1, 2)
        encoder_outputs = self.encoder.inference(embedded_inputs) #[1, 30, 512]
